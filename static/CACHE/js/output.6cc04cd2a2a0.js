...
const onValidateInput=(event)=>{event.target.disable=true
const inputs=[inputImage,inputTitle,inputDescription,inputCategory,inputDate,inputLevel]
const numberInputs=0
while(inputs.length>=numberInputs){if(inputs[i].value?.length>0){numberInputs=numberInputs+1}}
if(numberInputs===6){event.target.disable=false
